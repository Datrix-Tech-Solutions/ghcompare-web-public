...
<template>
    <div class="max max-w-lg mx-auto bg-white p-10 rounded-lg">
        <!-- summary -->
        <h3 class="text-2xl font-bold text-center mb-5 text-gray-600">Summary</h3>

        <!-- Cover details -->
        <div class="flex justify-between">
            <h3 class="text-xl font-semibold text-primary mb-3">Cover Details</h3>
            <router-link :to="{ name: 'MotorInsurance', query: { form: 'cover_details' } }">
        ...
        <div class="flex justify-between">
            ...
            <router-link :to="{ name: 'MotorInsurance', query: { form: 'vehicle_details' } }">
                <PencilSquareIcon class="w-5 h-5 inline text-primary cursor-pointer" />
            </router-link>
        </div>
        <p class="flex justify-between">
